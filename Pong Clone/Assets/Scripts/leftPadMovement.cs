...
public class leftPadMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //control the paddle
         if (Input.GetKey(KeyCode.W) && transform.position.y < yBorder) { //when O is pressed and still inside edge
            velocity.y = speed;      //move the speed up
            }
            else if (Input.GetKey(KeyCode.S) && transform.position.y > -yBorder){ //when L is pressed and still inside edge
